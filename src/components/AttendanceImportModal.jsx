  ...
  const processMatrixFormat = (jsonData, headers, headerRowIdx, year, month) => {
    ...
    for (let r = headerRowIdx + 1; r < jsonData.length; r++) {
      ...
      dateCols.forEach(({ day, idx }) => {
        ...
        if (extractedTimes.length > 0) {
          if (!mergedData[key]) {
            mergedData[key] = { emp: currentSysEmp, day, times: [], rawVal: cellStr }
          }
          mergedData[key].times.push(...extractedTimes)
        } else {
          // Xử lý giá trị chấm công trực tiếp: 0, 1, 0.5, P, X...
          const upper = cellStr.toUpperCase()
          let cong = 0
          if (upper === '1' || upper === 'X') cong = 1.0
          else if (upper === '0.5') cong = 0.5
          else if (upper === 'P') cong = 1.0
          else {
    ...
    Object.values(mergedData).forEach(item => {
      ...
      if (item.isCodeOnly) {
        const directStats = {
          checkIn: item.directCong > 0 ? '08:00' : '',
          checkOut: item.directCong > 0 ? '17:00' : '',
          hours: item.directCong * 8,
