...
test('PDF print HTML preserves Vietnamese report data and escapes untrusted text', () => {
  const html = buildAttendancePrintHtml({
    ...
    companyName: 'Công ty Demo B & <script>alert(1)</script>',
    month: '2026-09',
    filterLabel: 'Nhân sự: Nguyễn Thị Ánh',
    exportedAt: '14/09/2026 10:30',
    headers: ['Họ tên', 'Công ty'],
    rows: [['Nguyễn Thị Ánh', 'Công ty Demo B & <img src=x>']],
    tableMode: 'list'
  })
  ...
  assert.match(html, /Công ty Demo B &amp; &lt;script&gt;alert\(1\)&lt;\/script&gt;/)
  assert.match(html, /Công ty Demo B &amp; &lt;img src=x&gt;/)
